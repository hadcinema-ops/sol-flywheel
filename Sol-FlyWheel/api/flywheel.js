...
const SECRET_B58 = process.env.SECRET_KEY_B58; // base58 private key for ops wallet
const TOKEN_MINT_RAW = process.env.TOKEN_MINT; // may include trailing 'pump'
const SLIPPAGE_BPS = Number(process.env.SLIPPAGE_BPS || 100);
const SOL_BUFFER = Number(process.env.SOL_BUFFER || 0.02);
const CRON_KEY = process.env.CRON_KEY || ""; // optional shared secret for /api/flywheel
...
function authorized(req) {
  ...
  const key = (req.headers['x-cron-key'] || req.query?.key || req.headers['x-cronkey'] || "").toString();
  return key === CRON_KEY;
}

async function claimCreatorFees(conn, signer) {
  // PumpPortal: "collectCreatorFee" local tx you sign and send
...
async function sendAllTokensToIncinerator(conn, signer, mint) {
  ...
  const bal = acc.amount; // bigint
  if (bal === 0n) return null;

  // Create/get incinerator ATA (we fund creation if needed)
...
async function main() {
  ...
  const signer = new web3.Keypair.fromSecretKey(bs58.decode(SECRET_B58));
  const mint = new web3.PublicKey(normalizeMint(TOKEN_MINT_RAW));

  // 1) claim fees (safe no-op if none)
  let claimedSig = null;
  try {
    const claimed = await claimCreatorFees(conn, signer);
    if (claimed && claimed.sig) claimedSig = claimed.sig;
  } catch (e) {
    // ignore errors here; next run can pick it up
  }

  // 2) compute spendable SOL
  ...
  const headroom = Math.ceil(0.002 * web3.LAMPORTS_PER_SOL); // safety margin
  const spendable = balLamports - bufferLamports - headroom;

  let swapSig = null;
  if (spendable > 0) {
    try {
      swapSig = await jupSwapSOLtoToken(conn, signer, mint, spendable, SLIPPAGE_BPS);
    } catch (e) {
      // ignore; next run will retry
    }
  }

  // 3) burn to incinerator (black-hole)
  let burnSig = null;
  try {
    burnSig = await sendAllTokensToIncinerator(conn, signer, mint);
  } catch (e) {
    // ignore; next run will retry
  }
